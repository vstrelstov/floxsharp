...
module Scanner =
    let private ignoredSymbols =
        [| ' '; '\r'; '\t' |]

    let private doubleTokens =
    ...
    let scanTokens (source: string) =
        ...
        let createToken tokenType =
            createLexemedToken tokenType String.Empty

        let rec loop source tokens =
            let matchNext expected =
                peekNextSymbol source = (Some expected)
            ...
            let createTokenByNextSymbol expectedSymbol matchTokenType mismatchTokenType =
                if matchNext expectedSymbol then
                    ...
                    createToken mismatchTokenType

            let createStringToken =
                let lexeme =
                    getLexeme (tryTail source) (fun c ->
                        if c = '\n' then
            ...
            let scanToken currentChar =
                ...
                | _ ->
                    if Char.IsDigit currentChar then
                        ...
                        raise <| InterpreterException(lineNumber, String.Empty, "Unexpected character")

            let postprocessStringToken src lexeme =
                let newSource =
                    List.skip (String.length lexeme) src
            ...
            | head :: tail ->
                ...
                | x when Array.contains x doubleTokens -> loop (tryTail tail) (tokens @ [ newToken ])
